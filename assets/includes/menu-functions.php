...
/*menu superior Mobil*/
if (!function_exists('menu_superior_mobil')) {

    // Register Navigation Menus
    function menu_superior_mobil()
    {

        $locations = array(
            'menu-superior-mobil' => __('menu-superior-mobil', 'menu-superior-mobil'),
        );
        register_nav_menus($locations);
    }
    add_action('init', 'menu_superior_mobil');
}

/*clases para li item */
function atg_menu_classes_mobil($classes, $item, $args)
{
    if ($args->theme_location == 'menu-superior-mobil') {
        $classes[] = 'nav-item';
    }
    return $classes;
}
add_filter('nav_menu_css_class', 'atg_menu_classes_mobil', 1, 3);
/*clases para li item */
